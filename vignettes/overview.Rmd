...
```{r setup, warning = FALSE}
  ...
  # What crs to use for spatial analysis?
...
Due to the loose definition of [context] (see below), the definitions of site, visit, record and taxa can change through the cleaning process. 

* sites are spatial locations. they may be defined by latitiude, longitude, easting, northing and/or cell. These may be duplicated before exclusive application of context. The are not necessarily defined by all spatial concepts within context at all stages of the cleaning process.
...
Throughout the series of `env`Packages the concept of _context_ is used extensively, and at least currently, somewhat loosely.

With respect to 'loosely': context may be defined by, say, `c("lat", "long", "cell", "year", "month")`. At various stages through the cleaning process not every one of those variables may be applicable. After running `add_raster_cell` the variable `lat` and `long` may be removed (depending on the `add_xy` argument). However `context` can still be used in full in cleaning steps (via the consistent use of `tidyselect::any_of` in `envClean` functions).

Note that `context` is first applied _exclusively_ only when taxonomy is filtered (see examples below). Another way to say this: extraneous fields/columns beyond `context` are maintained until taxonomy is filtered. However, even at that point, extra columns can be specified (see [Filter taxonomy]). Thus, if a cleaning process does not include `filter_taxa`, running, say, `dplyr::distinct(across(any_of(context)))` may be useful.
