...
```{r context_example, echo = FALSE}
  ...
  kable(context_example
        , caption = "Context example. If context is `location` and `year`, only upon running `filter_taxa` will `data_source` drop out and the dataframe will collapse to one line (implemented by `dplyr::distinct(across(any_of(context)))`"
        )
