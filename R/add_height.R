...
add_height <- function(df
  ...
  site_use_height <- df |>
    ...
    dplyr::summarise(site_use_height = get(ht_func)(site_use_height
                                                    , na.rm = TRUE
                                                    )
                     ) |>
    dplyr::ungroup()
  ...
  pca_use_height <- df |>
    ...
    dplyr::left_join(site_use_height |>
                       ...
                       dplyr::summarise(pca_use_height = get(ht_func)(site_use_height
                                                                      , na.rm = TRUE
                                                                      )
                                        ) |>
                       dplyr::ungroup()
  ...
  taxa_use_height <- site_use_height |>
    dplyr::group_by(taxa) |>
    dplyr::summarise(taxa_use_height = get(ht_func)(site_use_height
                                                    , na.rm = TRUE
                                                    )
                     ) |>
    dplyr::ungroup()
