...
* the following text in a taxa name was removed from the name:
...
The taxa used in analyses were always binomial (taxa recorded at taxonomic level below species were grouped at species level (e.g. _Eucalyptus leucoxylon_ ssp. _pruinosa_ was changed to _Eucalyptus leucoxylon_). Any record that could not be matched to a name at `r rank_to_target` level, or better, was removed. Table \@ref(tab:taxHier) shows each taxonomic rank and the count of taxa matched at that rank.
...
The [appendix] Table \@ref(tab:taxonomy) maps names retrieved from data sources to those used in analyses.
